...
exports.mainAccess = async (req, res) => {
  ...
  if (Object.keys(req.query).length === 0) {
    ...
    queryString += "AND rating BETWEEN " + (randRating - 1) + " AND " + (randRating + 1) + " ORDER BY DBMS_RANDOM.VALUE OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY"
  }
  // If an ID gets passed in, handle that as a priority
    ...
    queryString += "AND puzzleid = '" + req.query.id + "' OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY"
  }
  ...
  // PLAYERMOVES
    ...
    if (req.query.playerMoves) {
      queryString += " AND(LENGTH(MOVES) - LENGTH(replace(MOVES, ' ', '')) + 1) = " + (parseInt(req.query.playerMoves) * 2)
    }
    ...
    queryString += " AND " + parseInt(rating) + " BETWEEN RATING - RATINGDEVIATION AND RATING + RATINGDEVIATION "
    queryString += " ORDER BY DBMS_RANDOM.VALUE OFFSET 0 ROWS FETCH NEXT " + limit + " ROWS ONLY"
  }
  let connection
  try {
